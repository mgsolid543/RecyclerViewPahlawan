...
public class PahlawanData {
    ...
    private static int[] photoPahlawan = {
    ...
    static ArrayList<Pahlawan> getListData() {
        ...
        for (int position = 0; position < namaPahlawan.length; position++) {
            Pahlawan pahlawan = new Pahlawan();
            pahlawan.setNama(namaPahlawan[position]);
            pahlawan.setDetail(detailPahlawan[position]);
            pahlawan.setPhoto(photoPahlawan[position]);
            list.add(pahlawan);
        }
        return list;
    }
